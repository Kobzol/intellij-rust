...
package org.rust.ide.refactoring.extractFields
...
class ExtractFieldsAction : CodeInsightAction() {

    private val extractFieldsHandler: ExtractFieldsHandler = ExtractFieldsHandler()

    override fun getHandler(): CodeInsightActionHandler = extractFieldsHandler

    override fun isValidForFile(project: Project, editor: Editor, file: PsiFile): Boolean =
        extractFieldsHandler.isValidFor(editor, file)
}

class ExtractFieldsHandler : LanguageCodeInsightActionHandler {
    ...
    override fun invoke(project: Project, editor: Editor, file: PsiFile) {
        ...
        extractFields(structItem, editor)
    }

    private fun extractFields(structItem: RsStructItem, editor: Editor) {
        checkWriteAccessNotAllowed()
        val chosenFields = showFieldsChooser(structItem.project, structItem) ?: return
        runWriteAction {
            extractFields(structItem, chosenFields, editor)
        }
    }

    private fun extractFields(structItem: RsStructItem, selectedFields: List<ConstructorArgument>, editor: Editor) {
