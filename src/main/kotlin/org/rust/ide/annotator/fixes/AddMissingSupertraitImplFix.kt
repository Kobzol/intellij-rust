...
class AddMissingSupertraitImplFix(implItem: RsImplItem) : LocalQuickFixAndIntentionActionOnPsiElement(implItem) {
    ...
    override fun invoke(
        ...
        val psiSubst = pathPsiSubst(traitRef.path, trait.element)
        collectSuperTraits(trait, traitRef, psiSubst, traits, mutableSetOf())

        for ((superTrait, ref) in traits) {
            if (!implLookup.canSelect(TraitRef(type, superTrait))) {
                implementTrait(impl, superTrait, ref, typeRef, trait, traitRef)
            }
        }
    }
}
...
private fun implementTrait(
    context: RsImplItem,
    superTrait: BoundElement<RsTraitItem>,
...
private fun collectSuperTraits(
    ...
    trait.element.typeParamBounds?.polyboundList?.forEach {
        ...
        collectSuperTraits(superTrait.substitute(trait.subst), traitRef, subst, ordered, visited)
    }
}
